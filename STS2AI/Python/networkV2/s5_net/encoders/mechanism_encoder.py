"""Mechanism Encoder: 表示"战斗进程走到哪了"。

输入: mechanism_bank tokens
输出: mechanism_tokens (B, L_mech, d_model)
"""

from __future__ import annotations

import torch.nn as nn

from networkV2.s5_net.encoders.common import TransformerBlock
from networkV2.s5_net.tokenizer import BankTensor


class MechanismEncoder(nn.Module):
    def __init__(self, d_model: int = 384, n_heads: int = 4, n_layers: int = 2, dropout: float = 0.1):
        super().__init__()
        self.layers = nn.ModuleList([
            TransformerBlock(d_model, n_heads, d_model * 2, dropout)
            for _ in range(n_layers)
        ])

    def forward(self, mech_bt: BankTensor) -> BankTensor:
        x = mech_bt.features
        mask = mech_bt.mask
        for layer in self.layers:
            x = layer(x, mask)
        return BankTensor(x, mask, mech_bt.bank_name)
