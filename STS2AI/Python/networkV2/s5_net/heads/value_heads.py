"""Value Heads: 7 个独立的价值评估头，各自独立投影。

1. fight_win_value: 战斗赢面 (sigmoid → [0, 1])
2. run_value:       整局赢面   (sigmoid → [0, 1])
3. expected_hp_loss: 期望掉血 (softplus → [0, +inf))
4. survival_2turn:  近 2 回合生存概率 (sigmoid → [0, 1])
5. tempo_value:     节奏优势 (tanh → [-1, 1])
6. turn_damage_lookahead: 本回合从此 step 起预期剩余总伤害 (softplus → [0, +inf))
   —— 强制网络学"先打这张然后这回合还能打多少伤害"，是 combo/牌序学习的关键监督。
7. turn_block_lookahead:  本回合从此 step 起预期剩余总挡伤 (softplus → [0, +inf))
"""
...
class ValueHeads(nn.Module):
    def __init__(self, d_model: int = 384, hidden_dim: int = 256):
        ...
        self.run_value = _SingleValueHead(d_model, hidden_dim, "sigmoid")
        ...
        self.turn_block = _SingleValueHead(d_model, hidden_dim, "softplus")

    def forward(self, decision_repr: torch.Tensor) -> ValueOutputs:
        return ValueOutputs(
            fight_win=self.fight_win(decision_repr),
            run_value=self.run_value(decision_repr),
            ...
            turn_block_lookahead=self.turn_block(decision_repr),
        )
