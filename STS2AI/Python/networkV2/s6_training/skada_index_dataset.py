...
class SkadaIndexFetcher:
    ...
    def sample_clean_runs(
        ...
        *,
        ...
        默认 `v0.103.2` — 和当前 sim 版本精确对齐。
        skada 混了 v0.99 老版本 + v0.103.0 / .1 / .2 三个 patch,老版本的
        encounter/card 可能被 rebalance 或改名,用不匹配版本 replay 会引入数据污染
        (deck 合法性 / encounter 胜率分布都可能偏差)。传 None 不过滤版本。
        """
        con = sqlite3.connect(str(self.index_db))
