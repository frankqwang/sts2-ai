"""Level 2: Power → RuleModifier 自动映射规则。
...
运行时 modifier_compiler 会扫描每个敌人的 power 列表，
对匹配的 power 自动生成 RuleModifier token。

添加新规则: 在 AUTO_MODIFIER_RULES dict 中加一条即可。
"""
...
#
# 运行时 compiler 会用 enemy 的 power stacks 填充 current_value
# ---------------------------------------------------------------------------

AUTO_MODIFIER_RULES: dict[str, ModifierPrimitive] = {
    # ---- 受击触发 ----
    "thorns": OnHitTrigger(
...
def compile_auto_modifiers(enemy: EnemyRuntime) -> list[ModifierPrimitive]:
    """根据敌人的 power 列表自动编译 Level 2 modifier。
    ...
    for power_name, stacks in enemy.powers.items():
        ...
        if lower in LEVEL1_NUMERIC_POWERS:
            continue
        template = AUTO_MODIFIER_RULES.get(lower)
        if template is None:
            continue
        # 复制模板并填充运行时值
        mod = replace(
