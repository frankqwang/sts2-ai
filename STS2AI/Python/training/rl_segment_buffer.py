"""非战斗 PPO 训练的半马尔可夫段缓冲区。

一个"段"跨越从一个非战斗决策点到下一个的范围，可能穿越一场或多场
战斗遭遇。这自然地将战斗结果传播回导致它们发生的非战斗决策。

GAE 使用 gamma^seg_len 而非逐步的 gamma，这压缩了有效时间视野并改善
了信用分配。
"""
...
from network.state_features import MAX_ACTIONS, StructuredActions, StructuredState
