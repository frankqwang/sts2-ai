...
Examples:
    cd STS2AI/Python
    python -m networkV2.s7_diagnostics.verify_save_load --backend godot-http --auto-launch
    python -m networkV2.s7_diagnostics.verify_save_load --backend headless-pipe --auto-launch
"""
...
from networkV2.s0_bridge.full_run_env import ApiBackedFullRunClient, BinaryBackedFullRunClient, FullRunClientLike, PipeBackedFullRunClient
from networkV2.s0_bridge.headless_sim_runner import DEFAULT_DLL_PATH, start_headless_sim, stop_process
from constants import ARTIFACTS_ROOT, REPO_ROOT
from networkV2.s7_diagnostics.test_simulator_consistency import state_summary
...
def stable_player_summary(state: dict[str, Any]) -> dict[str, Any]:
    ...
    extracted_player = {}
    try:
        from networkV2.s7_diagnostics.test_simulator_consistency import extract_player
