...
用法：
    ...
    python -m networkV2.s1_schema.card_tags --repo-root /path/to/sts2

    # 在代码中使用
    from networkV2.s1_schema.card_tags import load_card_tags, TAG_VOCAB
...
# card_tags.json lives at STS2AI/Python/data/card_tags.json.
# This file is at STS2AI/Python/networkV2/s1_schema/card_tags.py, so need 3 parents.
_DEFAULT_PATH = Path(__file__).parent.parent.parent / "data" / "card_tags.json"
...
def _find_repo_root() -> Path | None:
    ...
    for candidate in [
        here.parent.parent.parent.parent.parent,  # s1_schema/ → networkV2/ → Python/ → STS2AI/ → repo/
        here.parent.parent.parent.parent,         # s1_schema/ → networkV2/ → Python/ → STS2AI/
...
def load_card_tags(path: str | Path | None = None) -> dict[str, list[str]]:
    ...
    if not path.exists():
        ...
        raise FileNotFoundError(
            ...
            "Run `python -m networkV2.s1_schema.card_tags --repo-root <path>` to generate it."
        )
    data = json.loads(path.read_text())
    return data["card_tags"]
...
def card_feature_vector(card_id: str) -> list[float]:
    ...
    注意：不含 rarity / type / cost — 这些已在 token_bank_builder 的 coarse 11 维里。
