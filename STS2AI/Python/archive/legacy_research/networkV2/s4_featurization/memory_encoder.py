"""Memory 编码器：将 memory 对象转为 token 数值特征。
...
class MemoryEncoder:
    ...
    def encode_turn_prefix_summary(self, prefix: TurnPrefixMemory) -> list[float]:
    ...
    def encode_played_action(self, action: PlayedAction) -> list[float]:
        """编译单个已打出动作为数值向量。"""
        return [
    ...
    def encode_combat_memory(self, mem: CombatMemory) -> list[float]:
        """编译战斗长程记忆为数值向量。
    ...
    def encode_run_build_memory(self, mem: RunBuildMemory) -> list[float]:
        """编译整局慢变量为数值向量。"""
        return [
            # 构筑画像
