"""Token Bank Builder：将编译好的 schema 对象组装成 UnifiedTokenBanks。
...
from networkV2.s4_featurization.mechanism_inferer import ActiveMechanism
from networkV2.s4_featurization.memory_encoder import MemoryEncoder
from networkV2.s4_featurization.inventory_rules import relic_feature_vector, potion_feature_vector
from networkV2.s1_schema.card_tags import FUNCTIONAL_TAGS, NUM_FUNCTIONAL_TAGS
...
class TokenBankBuilder:
    """将编译好的 schema 对象组装成 UnifiedTokenBanks。"""

    def __init__(self) -> None:
        self._mem = MemoryEncoder()
    ...
    def _assemble_shared(
        ...
        shared.build_bank.add(Token(
            numeric=self._mem.encode_run_build_memory(rbm),
    ...
    def _assemble_combat(
        ...
        for i, action in enumerate(turn_prefix.played_actions):
            cb.turn_prefix_bank.add(Token(
                numeric=self._mem.encode_played_action(action),
                token_type=TK_PLAYED_ACTION,
                order=i,
            ))
        cb.turn_prefix_bank.add(Token(
            numeric=self._mem.encode_turn_prefix_summary(turn_prefix),
            token_type=TK_TURN_SUMMARY,
        ))

        # --- combat_memory_bank ---
        cb.combat_memory_bank.add(Token(
            numeric=self._mem.encode_combat_memory(combat_memory),
