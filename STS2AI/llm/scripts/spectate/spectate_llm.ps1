...
try {
    ...
    Write-Host "  python .\llm\scripts\viewers\replay_trace.py --trace `"$traceFile`" --follow" -ForegroundColor Yellow

    $spectateProc.WaitForExit()
    $exitCode = [int]$spectateProc.ExitCode
    $metricsScript = Join-Path $llmRoot "scripts\viewers\summarize_metrics.py"
