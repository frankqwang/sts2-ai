"""Run teacher reviews for sampled teacher candidate states.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="deepseek (default) / kimi / claude_cli",
    )
...
def main() -> int:
    ...
    key_env, key = _api_key(args.api_key_env) if args.provider == "kimi" else (args.api_key_env, "")
    ...
    if args.dry_run or not provider_ready:
        status = "dry_run" if args.dry_run else ("no_api_key" if args.provider == "kimi" else "no_claude_cli")
    else:
        status = "running"
        for group_index, group in enumerate(batch_rows):
            ...
            try:
                ...
                calls_after = count_recorded_api_calls(usage_path, provider=args.provider)
            print(
                json.dumps(
                    {
                        "group": group_index + 1,
