...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-stages",
        default="CHOMPERS;SLIMES,CULTISTS;BOWLBUGS,GREMLIN;skada_floor_06,skada_floor_07,skada_floor_08;",
    )
...
def _post_eval_artifacts(
    *,
    ...
    review_cmd = [
        str(python_exe),
        "-m",
        "llm.scripts.analysis.review_step_trace",
        "--trace",
        str(trace_path),
        "--out-dir",
        str(review_dir),
    ...
    viewer_cmd = [
        str(python_exe),
        "-m",
        "llm.scripts.viewers.trace_viewer_html",
        "--trace",
        str(trace_path),
        "--out",
        str(viewer_path),
...
def _spectate_command(
    *,
    ...
    command = [
        ...
        "Bypass",
        "-File",
        str(LLM_ROOT / "scripts" / "spectate" / "spectate_llm.ps1"),
        "-StopExistingGodot",
        "-AdapterDir",
        str(combat_adapter),
        "-CombatAdapterDir",
...
def _train_command(
    *,
    ...
    command = [
        str(python_exe),
        "-m",
        "llm.scripts.automation.self_train_loop",
        "--current-adapter",
        str(current_adapter),
        "--python-exe",
        str(python_exe),
