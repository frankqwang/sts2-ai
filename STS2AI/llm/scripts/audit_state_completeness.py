...
def main() -> None:
    ...
    # 用 midrun build 开一场，数据最丰富（带遗物、升级卡）
    spec = ACT1_POOL[5]  # midrun build

    session = create_game_session(mode="combat", transport="pipe_proto", backend="sim", port=15710, auto_launch=True)
    try:
        state = session.reset(
    ...
    checklist.append((f"battle.draw_pile_cards (内容)", f"{len(draw)} cards", "NO - 缺失"))
    checklist.append((f"battle.discard_pile_cards (内容)", f"{len(discard)} cards", "NO - 缺失"))
    checklist.append((f"battle.exhaust_pile_cards (内容)", f"{len(exhaust)} cards", "NO - 缺失"))
