...
class _SearchTeacher:
    def label_request(self, request, runtime_factory=None, seed: str | None = None):
        return TeacherLabel(
            ...
            metadata={"teacher": "fake"},
        )


class SearchGuidedCollectTests(unittest.TestCase):
    def test_targeted_search_guidance_overrides_student_greedy_action(self):
        selector = SearchGuidedActionSelector(
            search_teacher=_SearchTeacher(),
            queue_builder=TeacherQueueBuilder(TeacherConfig()),
    ...
    def test_search_only_collect_uses_search_policy_and_marks_transition(self):
        ...
        transitions = collector.collect(
            ...
            max_steps=2,
            temperature=0.0,
            search_self_play_factory=lambda _port=None: SearchSelfPlaySelector(search_teacher=_SearchTeacher()),
        )
        self.assertEqual(len(transitions), 1)
        self.assertEqual(transitions[0].action_index, 1)
