...
def compute_losses(output: ZeroNetOutput, batch, weights: LossWeights) -> LossBreakdown:
    ...
    uncertainty_loss = F.mse_loss(output.uncertainty, batch.uncertainty_target)
    total = (
...
def _compute_policy_loss(output: ZeroNetOutput, batch) -> torch.Tensor:
    ...
    losses = []
    if teacher_mask.any():
        logits = output.policy_logits[teacher_mask]
    ...
    imitation_loss = F.cross_entropy(output.policy_logits, batch.behavior_action_index, reduction="none")
    weighted = (imitation_loss * batch.sample_weight).mean()
...
def _compute_ranking_loss(output: ZeroNetOutput, batch) -> torch.Tensor:
    ...
    logits = output.policy_logits[mask]
    if logits.size(1) <= 1:
        return torch.zeros((), device=output.policy_logits.device)
    best_idx = batch.teacher_best_action_index[mask]
    best = logits.gather(1, best_idx.unsqueeze(1)).squeeze(1)
    mean_other = logits.masked_fill(
        F.one_hot(best_idx, num_classes=logits.size(1)).bool(),
        0.0,
    ).sum(dim=1) / (logits.size(1) - 1)
    margin = 0.1
