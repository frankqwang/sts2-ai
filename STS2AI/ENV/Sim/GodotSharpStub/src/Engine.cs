...
public partial class FileAccess : RefCounted, IDisposable
{
    ...
    private Error _lastError = Error.Ok;
    ...
    public static FileAccess? Open(string path, ModeFlags flags)
    {
        ...
        {
            // Mirror Godot's "best effort" file open failure without throwing.
            return null;
        }
    }
    ...
    public static ulong GetSize(string path)
    {
        try
        {
            return (ulong)new System.IO.FileInfo(ProjectSettings.GlobalizePath(path)).Length;
        }
        catch
        {
            return 0;
    ...
    public Error GetError() => _lastError;
