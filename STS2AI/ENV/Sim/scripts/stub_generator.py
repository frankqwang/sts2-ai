...
class CSharpScanner:
    ...
    def scan_all_type_namespaces(self):
        ...
        extra_dirs = [
            self.project_root / "STS2AI" / "ENV" / "Sim" / "GodotSharpStub" / "src",
            self.project_root / "STS2AI" / "ENV" / "Sim" / "HeadlessSim",
        ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 当前 HeadlessSim 项目位置
    output_path = project_root / "STS2AI" / "ENV" / "Sim" / "HeadlessSim" / "GeneratedStubs.cs"
    ...
    # CS0246 / CS1061: Scope 类型和扩展 API 在 headless host 里需要最小兼容。
    ...
    content = content.replace(
        ...
        "public static void AddBreadcrumb(string message, string category= \"app\", MegaCrit.Sts2.Core.Debug.BreadcrumbLevel level = MegaCrit.Sts2.Core.Debug.BreadcrumbLevel.Info) { }"
    )
    content = content.replace(
        "public static void CaptureMessage(string message, object? level = null, Action<Sentry.Scope>? configureScope= null) { }",
        "public static void CaptureMessage(string message, MegaCrit.Sts2.Core.Debug.SentryLevel level = MegaCrit.Sts2.Core.Debug.SentryLevel.Info, Action<Sentry.Scope>? configureScope= null) { }"
    )
    if 'namespace Sentry' not in content:
        content += (
            ...
            "    {\n"
            "        public void AddAttachment(byte[] data, string fileName) { }\n"
            "    }\n"
            "    public static class ScopeExtensions\n"
            "    {\n"
            "        public static void SetFingerprint(this Scope scope, params string[] values) { }\n"
    ...
    csproj = str(project_root / "STS2AI" / "ENV" / "Sim" / "HeadlessSim" / "HeadlessSim.csproj")
    import subprocess
