...
public class SerializableRoom : IPacketSerializable
{
	...
	[JsonSerializeCondition(SerializationCondition.SaveIfNotCollectionEmptyOrNull)]
	public Dictionary<ulong, SerializablePlayerCombatState> PlayerCombatStates { get; set; } = new Dictionary<ulong, SerializablePlayerCombatState>();

	/// <summary>
	/// Mid-combat creature states (enemies + player creatures).
	/// Captures HP, block, and powers for MCTS save/load fidelity.
	/// </summary>
	[JsonPropertyName("creature_states")]
	[JsonSerializeCondition(SerializationCondition.SaveIfNotCollectionEmptyOrNull)]
	public List<SerializableCreatureState> CreatureStates { get; set; } = new();

	/// <summary>Turn number at save time (0-based). Used to skip to correct turn on load.</summary>
	...
	public void Serialize(PacketWriter writer)
	{
		...
		writer.WriteInt(PlayerCombatStates.Count);
		foreach (var (netId, pcs) in PlayerCombatStates)
		{
			writer.WriteULong(netId);
			writer.Write(pcs);
		}
		writer.WriteList(CreatureStates);
		writer.WriteInt(TurnNumber);
		writer.WriteInt((int)CurrentSide);
	}
	...
	public void Deserialize(PacketReader reader)
	{
		...
		// PlayerCombatStates: added for mid-combat save/load (MCTS).
		// Guard with try-catch for backward compatibility with older packet data.
		try
		{
			int num3 = reader.ReadInt();
			PlayerCombatStates = new Dictionary<ulong, SerializablePlayerCombatState>(num3);
			for (int k = 0; k < num3; k++)
			{
				ulong key3 = reader.ReadULong();
				PlayerCombatStates[key3] = reader.Read<SerializablePlayerCombatState>();
			}
		}
		catch
		{
			PlayerCombatStates = new Dictionary<ulong, SerializablePlayerCombatState>();
		}
		try
		{
			CreatureStates = reader.ReadList<SerializableCreatureState>();
			TurnNumber = reader.ReadInt();
			CurrentSide = (CombatSide)reader.ReadInt();
		}
		catch
		{
			CreatureStates = new List<SerializableCreatureState>();
			TurnNumber = 0;
