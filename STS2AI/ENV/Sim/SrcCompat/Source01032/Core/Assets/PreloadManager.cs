...
public static class PreloadManager
{
	...
	public static async Task LoadRoomTreasureAssets(ActModel actModel)
	{
		...
		if (!string.IsNullOrWhiteSpace(actModel.ChestSpineResourcePath))
		{
			assets.Add(actModel.ChestSpineResourcePath);
		}
		assets.AddRange(NTreasureRoom.AssetPaths ?? Array.Empty<string>());
		await LoadRoomAssets("Treasure Room", new _003C_003Ez__ReadOnlyList<string>(assets));
	}
	...
	{
		List<string> assets = new List<string>();
		if (!string.IsNullOrWhiteSpace(actModel.RestSiteBackgroundPath))
		{
			assets.Add(actModel.RestSiteBackgroundPath);
		}

		foreach (RestSiteOption option in restSiteOptions ?? Array.Empty<RestSiteOption>())
		{
			try
			{
				assets.AddRange(option.AssetPaths ?? Array.Empty<string>());
			}
			catch
			{
				// HeadlessSim does not require UI asset fidelity here; some option asset
				// getters still assume scene-side collections exist and can throw.
			}
		}

		await LoadRoomAssets("RestSite Room", new _003C_003Ez__ReadOnlyList<string>(assets));
	}
	...
	{
		HashSet<string> hashSet = new HashSet<string>();
		foreach (IEnumerable<string> set in assetSets)
		{
			if (set == null)
			{
				continue;
