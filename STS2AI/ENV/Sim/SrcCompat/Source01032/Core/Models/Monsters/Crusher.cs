...
// Headless-safe overlay for Kaiser Crab's left arm. Same rationale as Rocket:
// background animations are optional, combat semantics are not.
public sealed class Crusher : MonsterModel
{
	...
	private async Task ThrashMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(ThrashDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
	}
	...
	private async Task BugStingMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(BugStingDamage).WithHitCount(BugStingTimes).FromMonster(this).WithNoAttackerAnim().Execute(null);
	...
	private async Task EnlargingStrikeMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(EnlargingStrikeDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
	}
	...
	private async Task GuardedStrikeMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(GuardedStrikeDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
