...
// Headless-safe overlay for Kaiser Crab's right arm. Upstream assumes the
// combat background scene always exists, but combat server / pure-sim runs can
// legitimately omit it. Gameplay still needs to proceed, so background-only
// animation hooks are treated as optional.
public sealed class Rocket : MonsterModel
{
	...
	private NKaiserCrabBossBackground? Background
	{
		get
		{
			...
			if (_background == null)
			{
				_background = NCombatRoom.Instance?.Background?.GetNodeOrNull<NKaiserCrabBossBackground>("%KaiserCrab");
			}
			return _background;
		}
	}
	...
	private async Task TargetingReticleMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(TargetingReticleDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
	}
	...
	private async Task PrecisionBeamMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(PrecisionBeamDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
	}
	...
	private async Task LaserMove(IReadOnlyList<Creature> targets)
	{
		...
		await DamageCmd.Attack(LaserDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
	}
