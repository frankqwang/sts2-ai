...
public sealed class SlumberingBeetle : MonsterModel
{
	...
	public override async Task AfterAddedToRoom()
	{
		...
		Marker2D? marker2D = HeadlessVisualCompat.TryGetSpecialNode<Marker2D>(
			HeadlessVisualCompat.TryGetCreatureNode(base.Creature),
			"%SleepVfxPos"
		);
		if (marker2D != null)
		{
			SleepingVfx = NSleepingVfx.Create(HeadlessVisualCompat.GetGlobalPositionOrZero(marker2D));
			HeadlessVisualCompat.TryAddChild(marker2D, SleepingVfx);
			HeadlessVisualCompat.TrySetPosition(SleepingVfx, Vector2.Zero);
		}
	...
	private async Task RolloutMove(IReadOnlyList<Creature> targets)
	{
		...
		if (nCreature != null)
		{
			NCreature? creatureNode = HeadlessVisualCompat.TryGetCreatureNode(LocalContext.GetMe(base.CombatState).Creature);
			Node2D? specialNode = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(nCreature, "Visuals/SpineBoneNode");
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(
					specialNode,
					Vector2.Left
						* (HeadlessVisualCompat.GetGlobalPositionOrZero(nCreature).X
							- HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X)
				);
			}
		}
