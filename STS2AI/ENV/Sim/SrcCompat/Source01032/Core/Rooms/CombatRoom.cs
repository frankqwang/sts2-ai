...
public class CombatRoom : AbstractRoom, ICombatRoomVisuals
{
	...
	private int? _savedTurnNumber;
	private CombatSide? _savedCurrentSide;
	...
	public new static CombatRoom FromSerializable(SerializableRoom serializableRoom, IRunState? runState)
	{
		...
		if (serializableRoom.TurnNumber > 0)
		{
			combatRoom._savedTurnNumber = serializableRoom.TurnNumber;
			combatRoom._savedCurrentSide = serializableRoom.CurrentSide;
		}
	...
	private void SerializeCombatPileState(SerializableRoom serializableRoom)
	{
		...
		serializableRoom.TurnNumber = CombatState.RoundNumber;
		serializableRoom.CurrentSide = CombatState.CurrentSide;
	}
	...
	private void RestoreSavedCombatPileState()
	{
		...
		// Restore creature states for both enemies and players.
		if (_savedCreatureStates != null && _savedCreatureStates.Count > 0)
		{
			List<Creature> remainingCreatures = CombatState.Creatures.ToList();

			Creature? TakeMatchingCreature(SerializableCreatureState savedCreature)
			{
				Creature? match = null;
				if (savedCreature.CombatId > 0)
				{
					match = remainingCreatures.FirstOrDefault(currentCreature => (currentCreature.CombatId ?? 0) == savedCreature.CombatId);
				}
				if (match == null && !string.IsNullOrWhiteSpace(savedCreature.Id))
				{
					match = remainingCreatures.FirstOrDefault(currentCreature => currentCreature.ModelId.Entry == savedCreature.Id);
				}
				if (match != null)
				{
					remainingCreatures.Remove(match);
				}
				return match;
			}
			...
			{
				Creature? creature = TakeMatchingCreature(savedCreature);
		...
		if (_savedTurnNumber.HasValue && _savedTurnNumber.Value > 0)
		{
			CombatState.RoundNumber = _savedTurnNumber.Value;
			CombatState.CurrentSide = _savedCurrentSide ?? CombatSide.Player;
		}
		...
		_savedTurnNumber = null;
		_savedCurrentSide = null;
	}
