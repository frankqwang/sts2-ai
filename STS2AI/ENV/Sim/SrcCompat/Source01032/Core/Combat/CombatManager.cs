    ...
    private async Task StartTurn(Func<Task>? actionDuringEnemyTurn = null)
    {
        ...
        foreach (Player item4 in playersStartingTurn)
        {
            if (LocalContext.NetId.HasValue)
            {
                ...
                await hookPlayerChoiceContext.WaitForPauseOrCompletionWithoutAssigningTask(task);
        ...
        if (_state.CurrentSide == CombatSide.Player)
        {
            ...
            {
                if (item5.PlayerCombatState != null && LocalContext.NetId.HasValue)
                {
            ...
            foreach (var item6 in playersStartingTurn.Zip(setupPlayerTurnContext))
            {
                ...
                var (player, _) = item6;
                var (hookPlayerChoiceContext3, setupPlayerTurnTask) = item;
                if (_state == null)
                {
                    return;
                }
                ...
                {
                    Task task3 = Hook.BeforePlayPhaseStart(hookPlayerChoiceContext3, setupPlayerTurnTask, _state, player);
                    await hookPlayerChoiceContext3.AssignTaskAndWaitForPauseOrCompletion(task3);
                }
            }
