...
public sealed class Tunneler : MonsterModel
{
	...
	private async Task BelowMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Right * (HeadlessVisualCompat.GetGlobalPositionOrZero(HeadlessVisualCompat.TryGetCreatureNode(targets[0])).X - HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X) * 3f);
			}
