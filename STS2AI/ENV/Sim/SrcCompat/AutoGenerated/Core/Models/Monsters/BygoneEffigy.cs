...
public sealed class BygoneEffigy : MonsterModel
{
	...
	private async Task SlashMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			Node2D specialNode = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode2, "Visuals/SpineBoneNode");
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Left * (vector.Value.X - HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode2).X - 300f));
			}
		}
