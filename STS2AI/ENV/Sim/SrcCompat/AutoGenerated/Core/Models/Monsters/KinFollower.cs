...
public sealed class KinFollower : MonsterModel
{
	...
	private async Task BoomerangMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Left * (HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode2).X - vector.Value.X) / HeadlessVisualCompat.GetBodyScaleOrOne(creatureNode2));
			}
		}
