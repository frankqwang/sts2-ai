...
public sealed class FuzzyWurmCrawler : MonsterModel
{
	...
	private async Task AcidGoop(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Left * (HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X - HeadlessVisualCompat.GetGlobalPositionOrZero(HeadlessVisualCompat.TryGetCreatureNode(targets.First())).X));
			}
		}
		IsPuffed = false;
