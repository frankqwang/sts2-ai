...
public sealed class ThievingHopper : MonsterModel
{
	...
	private async Task ThieveryMove(IReadOnlyList<Creature> targets)
	{
		...
		if (creatureNode != null)
		{
			...
			NCreature creatureNode2 = HeadlessVisualCompat.TryGetCreatureNode(creature);
			Node2D specialNode = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode, "Visuals/SpineBoneNode");
			if (specialNode != null)
			{
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Right * (HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode2).X - HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X));
			}
		}
		...
		foreach (CardModel item in cardsToSteal)
		{
			...
			{
				Marker2D specialNode2 = HeadlessVisualCompat.TryGetSpecialNode<Marker2D>(creatureNode, "%StolenCardPos");
				if (specialNode2 != null)
				{
					NCard nCard = NCard.Create(item);
					HeadlessVisualCompat.TryAddChild(specialNode2, nCard);
	...
	{
		HeadlessVisualCompat.TryGetCreatureNode(base.Creature)?.ToggleIsInteractable(on: false);
		SfxCmd.Play(FleeSfx);
