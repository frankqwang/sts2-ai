...
public sealed class TestSubject : MonsterModel
{
	...
	private async Task RespawnMove(IReadOnlyList<Creature> targets)
	{
		...
		await Cmd.Wait(0.8f);
		HeadlessVisualCompat.TryGetCreatureNode(base.Creature)?.SetDefaultScaleTo(1f + (float)Respawns * 0.1f, 0.1f);
	...
	{
		SetColor(Colors.White);
		HeadlessVisualCompat.TryAddBackCombatVfx(NTestSubjectBurnVfx.Create());
	...
	{
		HeadlessVisualCompat.TryGetSpecialNode<CanvasGroup>(HeadlessVisualCompat.TryGetCreatureNode(base.Creature), "%CanvasGroup")?.SetSelfModulate(color);
	}
