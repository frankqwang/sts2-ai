	...
	private async Task StickyShotMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			foreach (Creature target in targets)
			{
				...
				if (creatureNode != null && (!vector.HasValue || vector.Value.X > HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X))
				{
					vector = creatureNode.VfxSpawnPosition;
				}
			}
			Node2D node2D = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(HeadlessVisualCompat.TryGetCreatureNode(base.Creature), "Visuals/SpitTarget");
			if (node2D != null)
			{
				HeadlessVisualCompat.TrySetGlobalPosition(node2D, vector.Value);
			}
		}
		SfxCmd.Play(CastSfx);
