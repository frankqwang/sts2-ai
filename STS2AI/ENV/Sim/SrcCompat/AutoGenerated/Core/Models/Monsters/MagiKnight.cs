...
public sealed class MagiKnight : MonsterModel
{
	...
	private async Task MagicBombMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			foreach (Creature target in targets)
			{
				NCreature creatureNode = HeadlessVisualCompat.TryGetCreatureNode(target);
				if (!vector.HasValue || vector.Value.X > HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X)
				{
					vector = HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode);
				}
			}
			NCreature creatureNode2 = HeadlessVisualCompat.TryGetCreatureNode(base.Creature);
			Node2D specialNode = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode2, "Visuals/AttackDistanceControl");
			if (specialNode != null)
			{
				float x = HeadlessVisualCompat.GetCurrentBodyScaleXOrOne(creatureNode2);
				HeadlessVisualCompat.TrySetPosition(specialNode, Vector2.Left * ((HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode2).X - vector.Value.X - 600f) / x));
			}
		}
