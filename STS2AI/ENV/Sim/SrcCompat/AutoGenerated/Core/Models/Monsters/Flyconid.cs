...
public sealed class Flyconid : MonsterModel
{
	...
	private async Task VulnerableSporesMove(IReadOnlyList<Creature> targets)
	{
		if (TestMode.IsOff)
		{
			...
			HeadlessVisualCompat.TryGetCurrentBodyNode<NFlyconidSporesVfx>(creatureNode, "%VfxController")?.SetSporeTypeIsVulnerable(isVulnerable: true);
		}
		SfxCmd.Play(CastSfx);
	...
	private async Task FrailSporesMove(IReadOnlyList<Creature> targets)
	{
		...
		{
			NCreature creatureNode = HeadlessVisualCompat.TryGetCreatureNode(base.Creature);
			HeadlessVisualCompat.TryGetCurrentBodyNode<NFlyconidSporesVfx>(creatureNode, "%VfxController")?.SetSporeTypeIsVulnerable(isVulnerable: false);
		}
