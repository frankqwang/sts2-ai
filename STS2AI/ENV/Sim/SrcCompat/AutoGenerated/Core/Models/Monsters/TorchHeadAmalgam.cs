...
public sealed class TorchHeadAmalgam : MonsterModel
{
	...
	public override void OnDieToDoom()
	{
		if (TestMode.IsOff)
		{
			NCreature creatureNode = HeadlessVisualCompat.TryGetCreatureNode(base.Creature);
			if (creatureNode != null)
			{
				HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode, "Visuals/torch1Slot/fire1_small_green/light_small")?.SetVisible(visible: false);
				HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode, "Visuals/torch2Slot/fire2_small_green/light_small")?.SetVisible(visible: false);
				HeadlessVisualCompat.TryGetSpecialNode<Node2D>(creatureNode, "Visuals/torch3Slot/fire3_small_green/light_small")?.SetVisible(visible: false);
			}
		}
	}
	...
	{
		NCreature nCreature = HeadlessVisualCompat.TryGetCreatureNode(base.Creature);
		if (nCreature != null)
		{
			Node2D specialNode = HeadlessVisualCompat.TryGetSpecialNode<Node2D>(nCreature, "Visuals/LaserControlBone");
			if (specialNode != null)
			{
				NCreature creatureNode = HeadlessVisualCompat.TryGetCreatureNode(targets[0]);
				HeadlessVisualCompat.TryOffsetPosition(specialNode, Vector2.Left * (HeadlessVisualCompat.GetGlobalPositionOrZero(creatureNode).X - HeadlessVisualCompat.GetGlobalPositionOrZero(nCreature).X + 3000f));
			}
		}
