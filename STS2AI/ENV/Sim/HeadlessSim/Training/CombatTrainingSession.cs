...
public partial class CombatTrainingSession : Node
{
	...
	private async Task StartEpisodeAsync(Nodes.NGame game, CombatTrainingResetRequest? request)
	{
		...
		try
		{
			...
			RunState runState = await game.StartNewSingleplayerRun(character, shouldSave: false, ActModel.GetDefaultList(), Array.Empty<ModifierModel>(), seed, GameMode.Standard, ascensionLevel);
