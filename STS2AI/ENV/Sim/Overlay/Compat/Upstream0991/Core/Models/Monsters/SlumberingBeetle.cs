...
// Overlay replacement for src/Core/Models/Monsters/SlumberingBeetle.cs.
// Only AfterAddedToRoom() differs from upstream: the marker2D lookup now uses
// `NCombatRoom.Instance?` to tolerate headless sim runs where the GUI singleton
// is null. Without the null-safe operator the process NREs the first time a
// Slumbering Beetle is added to the room under headless mode. Every other
// member in this file mirrors upstream verbatim.
public sealed class SlumberingBeetle : MonsterModel
{
	...
	public override async Task AfterAddedToRoom()
	{
		...
		// Overlay-only diff: `NCombatRoom.Instance?.` instead of `NCombatRoom.Instance.`
		// so headless sim (no GUI room) returns marker2D=null and skips the vfx block
