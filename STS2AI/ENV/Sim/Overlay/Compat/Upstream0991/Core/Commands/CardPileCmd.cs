...
{
	private static bool SkipCombatPresentation => TestMode.IsOn || CombatSimulationRuntime.IsPureCombatSimulator;

	private static bool HasCombatRoomPresentation => !SkipCombatPresentation && NCombatRoom.Instance != null;

	private static bool HasGlobalUiPresentation => !SkipCombatPresentation && NRun.Instance?.GlobalUi != null;
	...
	public static async Task RemoveFromDeck(IReadOnlyList<CardModel> cards, bool showPreview = true)
	{
		foreach (CardModel card in cards)
		{
			...
			if (showPreview && LocalContext.IsMine(card) && HasGlobalUiPresentation)
			{
	...
	public static async Task RemoveFromCombat(IEnumerable<CardModel> cards, bool skipVisuals = false)
	{
		...
		if (cardNodes.Count != 0 && HasCombatRoomPresentation)
		{
	...
	public static async Task<IReadOnlyList<CardPileAddResult>> Add(IEnumerable<CardModel> cards, CardPile newPile, CardPilePosition position = CardPilePosition.Bottom, AbstractModel? source = null, bool skipVisuals = false)
	{
		...
		for (int i = 0; i < results.Count; i++)
		{
			...
			bool flag4;
			bool flag5;
			if (!SkipCombatPresentation && flag && !skipVisuals)
			{
		...
		if (cardsWithoutNodesChangingPiles.Count != 0 && !SkipCombatPresentation)
		{
			foreach (CardModel card4 in cardsWithoutNodesChangingPiles)
			{
				...
				Node vfxContainer = GetVfxContainerForPile(card4.Pile.Type);
	...
	{
		NCombatRoom room = NCombatRoom.Instance ?? throw new InvalidOperationException("Combat room presentation is not available.");
		NCard nCard = NCard.Create(card);
		room.Ui.AddChildSafely(nCard);
		nCard.UpdateVisuals(targetPileType, CardPreviewMode.Normal);
		if (!owningPlayerIsLocal)
		{
			nCard.Position = room.GetCreatureNode(card.Owner.Creature)?.IntentContainer.GlobalPosition ?? nCard.Position;
		}
	...
	private static Node? GetVfxContainerForPile(PileType pileType)
	{
		if (pileType == PileType.Deck)
		{
			return HasGlobalUiPresentation ? NRun.Instance.GlobalUi.TopBar.TrailContainer : null;
		}
		return HasCombatRoomPresentation ? NCombatRoom.Instance.CombatVfxContainer : null;
	}
	...
	public static async Task Shuffle(PlayerChoiceContext choiceContext, Player player)
	{
		...
		foreach (CardModel item2 in list)
		{
			if (!drawPileCards.Contains(item2))
			{
				...
				if (!CombatSimulationRuntime.IsPureCombatSimulator && (double)waitTimeAccumulator >= ((SceneTree)Engine.GetMainLoop()).Root.GetProcessDeltaTime())
				{
					await Cmd.Wait(num);
	...
	{
		if (!CombatSimulationRuntime.IsPureCombatSimulator && !SaveManager.Instance.SeenFtue("shuffle_ftue") && NModalContainer.Instance != null)
		{
