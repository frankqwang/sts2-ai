...
public static class CardSelectCmd
{
	...
	private static void RegisterCardSelectionPrompt(IEnumerable<CardModel> options, CardSelectorPrefs prefs, string mode)
	{
		if (Selector is ICardSelectionPromptSelector cardSelectionPromptSelector)
		{
			cardSelectionPromptSelector.RegisterCardSelection(options, prefs, mode);
		}
	}
	...
	public static async Task<CardModel?> FromChooseACardScreen(PlayerChoiceContext context, IReadOnlyList<CardModel> cards, Player player, bool canSkip = false)
	{
		...
		if (ShouldSelectLocalCard(player))
		{
			...
			if (Selector != null)
			{
				CardSelectorPrefs prefs = new CardSelectorPrefs(new LocString("", ""), canSkip ? 0 : 1, 1)
				{
					Cancelable = canSkip
				};
				RegisterCardSelectionPrompt(cards, prefs, "ChooseCard");
				result = (await Selector.GetSelectedCards(cards, prefs.MinSelect, prefs.MaxSelect)).FirstOrDefault();
			}
			else
			{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					...
					RegisterCardSelectionPrompt(options, prefs, "RewardSimpleGrid");
	...
	public static async Task<IEnumerable<CardModel>> FromSimpleGrid(PlayerChoiceContext context, IReadOnlyList<CardModel> cardsIn, Player player, CardSelectorPrefs prefs)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					RegisterCardSelectionPrompt(cards, prefs, "SimpleGrid");
	...
	public static async Task<IEnumerable<CardModel>> FromDeckForUpgrade(Player player, CardSelectorPrefs prefs)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					RegisterCardSelectionPrompt(list, prefs, "DeckUpgrade");
	...
	public static async Task<IEnumerable<CardModel>> FromDeckForTransformation(Player player, CardSelectorPrefs prefs, Func<CardModel, CardTransformation>? cardToTransformation = null)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					RegisterCardSelectionPrompt(list, prefs, "DeckTransform");
					enumerable = await Selector.GetSelectedCards(list, prefs.MinSelect, prefs.MaxSelect);
				}
				else
				{
	...
	public static async Task<IEnumerable<CardModel>> FromDeckForEnchantment(IReadOnlyList<CardModel> cards, EnchantmentModel enchantment, int amount, CardSelectorPrefs prefs)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				...
				if (Selector != null)
				{
					RegisterCardSelectionPrompt(list, prefs, "DeckEnchant");
					enumerable = await Selector.GetSelectedCards(list, prefs.MinSelect, prefs.MaxSelect);
				}
				else
				{
	...
	public static async Task<IEnumerable<CardModel>> FromDeckGeneric(Player player, CardSelectorPrefs prefs, Func<CardModel, bool>? filter = null, Func<CardModel, int>? sortingOrder = null)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector != null)
				{
					RegisterCardSelectionPrompt(list, prefs, "DeckGeneric");
					enumerable = await Selector.GetSelectedCards(list, prefs.MinSelect, prefs.MaxSelect);
				}
				else
				{
	...
	public static async Task<IEnumerable<CardModel>> FromHand(PlayerChoiceContext context, Player player, CardSelectorPrefs prefs, Func<CardModel, bool>? filter, AbstractModel source)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				if (Selector is IHandSelectionCardSelector handSelectionCardSelector)
				{
					handSelectionCardSelector.RegisterHandSelection(list, prefs, NPlayerHand.Mode.SimpleSelect.ToString());
				}
				result = ((Selector == null) ? (await NCombatRoom.Instance.Ui.Hand.SelectCards(prefs, filter, source)) : (await Selector.GetSelectedCards(list, prefs.MinSelect, prefs.MaxSelect)));
	...
	public static async Task<CardModel?> FromHandForUpgrade(PlayerChoiceContext context, Player player, AbstractModel source)
	{
		...
		else
		{
			...
			if (ShouldSelectLocalCard(player))
			{
				CardSelectorPrefs prefs = new CardSelectorPrefs(new LocString("gameplay_ui", "CHOOSE_CARD_UPGRADE_HEADER"), 1);
				if (Selector is IHandSelectionCardSelector handSelectionCardSelector)
				{
					handSelectionCardSelector.RegisterHandSelection(list, prefs, NPlayerHand.Mode.UpgradeSelect.ToString());
				}
				result = ((Selector == null) ? (await NCombatRoom.Instance.Ui.Hand.SelectCards(prefs, (CardModel c) => c.IsUpgradable, source, NPlayerHand.Mode.UpgradeSelect)).FirstOrDefault() : (await Selector.GetSelectedCards(list, 1, 1)).FirstOrDefault());
