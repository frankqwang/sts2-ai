...
public static partial class McpMod
{
    ...
    public static void Initialize()
    {
        try
        {
            ...
            // Start named pipe server for high-speed MCTS IPC (~50x faster than HTTP)
            StartPipeServer(port);
        }
        catch (Exception ex)
        {
    ...
    private static void HandleRequest(HttpListenerContext context)
    {
        try
        {
            ...
            {
                if (request.HttpMethod == "POST")
                    HandlePostFullRunEnvSaveState(response);
                else
                    SendError(response, 405, "Method not allowed");
            }
            ...
            {
                if (request.HttpMethod == "POST")
                    HandlePostFullRunEnvLoadState(request, response);
                else
                    SendError(response, 405, "Method not allowed");
            }
            ...
            {
                if (request.HttpMethod == "POST")
                    HandlePostFullRunEnvExportState(request, response);
                else
                    SendError(response, 405, "Method not allowed");
            }
            ...
            {
                if (request.HttpMethod == "POST")
                    HandlePostFullRunEnvImportState(request, response);
                else
                    SendError(response, 405, "Method not allowed");
            }
            ...
            {
                if (request.HttpMethod == "POST")
                    HandlePostFullRunEnvDeleteState(request, response);
                else
                    SendError(response, 405, "Method not allowed");
            }
            else
